...
- **The modal machinery moved out of this component when Sheet arrived** — showModal, the counted scroll lock, the Escape interception, the backdrop click and the report of a close nobody asked for are now `useModalDialog`, shared with Sheet. Every one of them exists because of a specific bug, several of them silent, and a second hand-written copy would have drifted on exactly those. Nothing about this component's behaviour changed — the same 27 tests pass against it.
